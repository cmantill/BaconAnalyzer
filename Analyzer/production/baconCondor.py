...
def submit_jobs(lofjobs):
    for sub_file in lofjobs:
        ...
        condor_file.write('Transfer_Input_Files = %s/src/BaconAnalyzer/Analyzer/production/cmsset_default.sh, %s.tgz, %s/bin/slc6_amd64_gcc530/runPFJetsToJUNIPR, %s/src/BaconAnalyzer/Analyzer/data.tgz\n'%(cmssw_base,cmssw_base,cmssw_base,cmssw_base))
...
for job_i in range(njobs):
    ...
    for subjob_i in range(njobs_per_file):
        ...
        for fil_i, fil in enumerate(files):
            ...
            exec_line_i = exec_line_i.replace('Output.root','Output_%d.dat'%fil_i)
        ...
        if options.eosoutdir:
            if njobs_per_file > 1:                    
                lOut = '/%s/%s_job%d_file%dto%d_subjob%d.dat'%(
                    options.eosoutdir, outfile, job_i, lIFile, lFFile, subjob_i)
            else:
                lOut = '/%s/%s_job%d_file%dto%d.dat'%(
