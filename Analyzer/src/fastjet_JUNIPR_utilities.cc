...
int get_mother(vector<int> intermediate_states, vector<PseudoJet> jets, int mother_index){
  ...
  for(unsigned int i=0; i< intermediate_states.size(); i++){
...
vector<double> get_daughter_momenta(PseudoJet daughter1, PseudoJet daughter2, PseudoJet ref){
  ...
  for(unsigned int i=0; i<soft_ETPM.size(); i++){
...
void convert_cluster_sequence_to_JUNIPR(ClusterSequence cs, ostream& outfile){
  ...
  // Branchings
  outfile << "B " ;
  for(unsigned int i =0; i<branchings.size(); i++){
    for(int j=0; j<4; j++){
  ...
  // Daughters
  outfile << "D " ;
  for(unsigned int i =0; i<daughter_momenta.size(); i++){
    for(int j=0; j<8; j++){
  ...
  // Mother momenta
  outfile << "P " ;
  for(unsigned int i =0; i<mother_momenta.size(); i++){
    for(int j=0; j<4; j++){
